...
# Traffic mode (NEW)
# ----------------------
# Set default to "butterfly" to run the new pattern; use "poisson" to restore old behavior
TRAFFIC_MODE = "butterfly"    # "butterfly" or "poisson"

# Butterfly traffic parameters (NEW)
BUTTERFLY_PERIOD   = 32       # slots per cycle (>= 2 * BUTTERFLY_ON_WIDTH)
BUTTERFLY_ON_WIDTH = 2        # slots per burst window (shorter = sharper bursts)
...
class CSMADQNEnv:
    ...
    def __init__(self, num_nodes, traffic_mode=TRAFFIC_MODE):
        self.num_nodes    = num_nodes
        self.traffic_mode = traffic_mode
        # optional partner map (useful if you later add destinations)
        self.partner = np.array([(i + (num_nodes // 2)) % num_nodes for i in range(num_nodes)], dtype=np.int32)
        self.reset()

    # ---------- arrivals: Poisson timers ----------
    def _reset_poisson_timers(self):
        lam = max(1e-6, LAMBDA_RATE)
        self.next_arrival_time = np.random.exponential(1/lam, self.num_nodes)

    # ---------- arrivals: Butterfly schedule (NEW) ----------
    def _butterfly_arrivals_this_slot(self, t):
        """
        Two short bursts per period:
          - Wave A at phase in [0, BUTTERFLY_ON_WIDTH): nodes 0..N/2-1
          - Wave B at phase in [P/2, P/2+BUTTERFLY_ON_WIDTH): nodes N/2..N-1
        """
        P = BUTTERFLY_PERIOD
        w = BUTTERFLY_ON_WIDTH
        phase = t % P
    ...
    def reset(self):
        self.time = 0
        self.cw   = CW_INIT
        # binary queue: one-packet buffer per node (0/1)
    ...
    def _apply_arrivals(self):
        if self.traffic_mode == "poisson":
            ...
            arrivals = self._butterfly_arrivals_this_slot(self.time)
            # keep one-packet buffer semantics
            self.queue = np.where((self.queue == 0) & (arrivals == 1), 1, self.queue)
    ...
    def step(self, action_multiplier):
        ...
        # Arrivals (after channel event within this slot)
...
# ======================================================
# Train DQN agent  (accepts traffic_mode)  [CHANGED]
# ======================================================
def dqn_train(traffic_mode=TRAFFIC_MODE):
    env = CSMADQNEnv(NUM_NODES, traffic_mode=traffic_mode)
    state_dim = 5
...
# ======================================================
# Static baseline (fixed CW)  (accepts traffic_mode)  [CHANGED]
# ======================================================
def run_static_baseline(cw_fixed=64, traffic_mode=TRAFFIC_MODE):
    env = CSMADQNEnv(NUM_NODES, traffic_mode=traffic_mode)
...
# ======================================================
# Run  (choose mode here)  [CHANGED]
# ======================================================
# Options: "butterfly" or "poisson"
RUN_MODE = "butterfly"

returns, thr_ad, lat_ad, en_ad, ut_ad, cw_trace = dqn_train(traffic_mode=RUN_MODE)
thr_st, lat_st, en_st, ut_st, thr_s_avg, lat_s_avg, en_s_avg, ut_s_avg = run_static_baseline(
    cw_fixed=64, traffic_mode=RUN_MODE
)
...
plt.title("Energy (J/bit) — average"); plt.ylabel("Energy per bit(J/bit)"); plt.grid(True, axis="y", alpha=0.3)
plt.tight_layout(); plt.show()
